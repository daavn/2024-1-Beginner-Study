...
- 답: 1은 main이 아닌 브랜치의 커밋들이 그대로 main 브랜치로 병합되는 반면, 2는 main이 아닌 브랜치의 커밋들이 하나로 합쳐진 다음 main으로 병합된다. 3은 main이 아닌 브랜치의 커밋들을 그대로 main에 적용한다.
...
- 답: develop 브랜치에 readme 파일이 추가되었다.
...
- 답: 다른 사람이 생성한 커밋을 함부로 삭제하기보다는, revert를 사용하여 커밋을 새로 생성하는 방식으로 안전하게 되돌릴 것이다.
